...
namespace SpringGUI
{
    public class UITree : UIBehaviour
    {
        ...
        public void Inject(List<UITreeData> listData)
        {
            if (null == m_container)
                getComponent();
            foreach (UITreeData Node in listData)
            {
                GameObject treeNode = cloneTreeNode();
                // TreeRootNode.Inject(Node);
                treeNode.transform.SetParent(m_container);
                treeNode.SetActive(true);
                treeNode.transform.localScale = Vector3.one;
                treeNode.GetComponent<UITreeNode>().Inject(Node);
            }
        }
